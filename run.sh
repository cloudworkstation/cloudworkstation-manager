...
envsubst '${STATE_BUCKET} ${STATE_KEY} ${AWS_REGION} ${VERSION} ${VPC_ID} ${TASK_SUBNETS} ${INSTANCE_SUBNETS} ${DESKTOP_ID} ${AMI_ID} ${MACHINE_USERNAME} ${NAMESPACE_ID} ${SECURITY_GROUP_ID} ${B64_USER_DATA}' < main.tf-template > main-${DESKTOP_ID}.tf
